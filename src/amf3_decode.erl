...
-record(refmap,
        {
          str   = ?DECODE_REFERENCE_TABLE_INIT() :: decode_reference_table(),
          obj   = ?DECODE_REFERENCE_TABLE_INIT() :: decode_reference_table(),
          trait = ?DECODE_REFERENCE_TABLE_INIT() :: decode_reference_table()
        }).

-type refmap_index() :: decode_reference_table_index().
-type refmap() :: #refmap{}.
-type refmap_get_result(ValueType) :: none|undefined|{value, ValueType}.

-type internal_result(ValueType) :: {ValueType, binary(), refmap()}.

-type decode_fn() :: fun((binary(), refmap()) -> internal_result(amf:amf_value())).
-type decode_object_fn() :: fun((U28::non_neg_integer(), binary(), refmap()) -> internal_result(amf:amf_value())).

-type object_trait() :: {undefined|amf:amf_string(), boolean(), [amf:amf_string()]}.
...
decode_impl(<<Bin/binary>>, _RefMap) ->
    ?THROW_PARTIAL(marker, Bin).

-spec decode_unknown(non_neg_integer(), binary(), refmap()) -> no_return().
decode_unknown(<<Bin/binary>>, _RefMap, Marker) -> ?THROW_INVALID(marker, {Marker, Bin}).
...
decode_double(<<N:8/binary, Bin/binary>>, _RefMap) -> ?THROW_UNSUPPORTED(double, {N, Bin});
...
decode_array(<<Bin/binary>>, RefMap) ->
    with_object_reference(
      ...
      fun (DenseElementCount, Bin1, RefMap1) ->
              ...
              case Members of
                  [] -> {Values, Bin3, RefMap3};
                  _  -> {amf:array(Values, Members), Bin3, RefMap3}
              end
      end).
...
decode_date(<<Bin/binary>>, RefMap) ->
    with_object_reference(
      ...
      fun (_U28, <<MilliSecs/float, Bin1/binary>>, RefMap1) -> 
          ...
          (_U28, <<Bytes:8/binary, Bin1/binary>>, _RefMap1) -> ?THROW_INVALID(date, {Bytes, Bin1});
...
decode_trait(<<RefIndex:27, 0:1>>, Bin, RefMap) ->
    case refmap_trait_get(RefMap, RefIndex) of
        none           -> ?THROW_INVALID(trait_reference, {RefIndex, Bin});
    ...
    case U29 of
        ...
        <<RefIndex:28, 0:1>> ->
            case refmap_str_get(RefMap, RefIndex) of
                ...
                none -> ?THROW_INVALID(string_reference, {RefIndex, Bin1})
            end
    end.
    ...
    case decode_u29(Bin) of
        {<<RefIndex:28,0:1>>, Bin1} ->
            case refmap_obj_get(RefMap, RefIndex) of
                none           -> ?THROW_INVALID(object_reference, {RefIndex, Bin1});
                undefined      -> ?THROW_UNSUPPORTED(circular_reference, {RefIndex, Bin1});
